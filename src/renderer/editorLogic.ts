...
function findFreeNodePosition(centerX: number, centerY: number, nodes: Node[]): { x: number; y: number } {
...
export function startConnect(state: EditorState, destinationNodeId: number, portType: EdgeType): EditorState {
  if (!state.connecting) {
